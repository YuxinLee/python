...
class Datacenter:
    ...
    def init_datacenter_network(self, core_switch_num):
    ...
    获取低于阈值的服务器列表
    """
    def get_low_resource_host_list(self):
    ...
    def get_candidate_host_list(self):
        active_host_list = self.get_active_host_list()
        candidate_host_list = []
        for i in active_host_list:
            if ((i.cpu_used * 1.0 / i.cpu_sum) > 0.2 and (i.mem_used * 1.0 / i.mem_sum) > 0.2):
    ...
    def get_avail_host_list(self):
        ...
        for i in active_host_list:
            ...
            if ((( host_cpu_used) * 1.0 / host_cpu_sum <= 0.6) and (( host_mem_used) * 1.0 / host_mem_sum <= 0.6)):
