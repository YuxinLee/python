...
def main():
    ...
    初始化工业云数据中心
    """
    d_ST = datacenter.Datacenter()
    d_ST.init_datacenter_network(2)
    g_ST = d_ST.g

    print("工业云数据中心的资源为：")
    d_ST.show_datacenter_network()
    ...
    for i in range(user_num):
        ...
        for vm in user_vm_list:
            algorithm_Random.placement_Random(d_ST.host_list, vm)
        algorithm_VMSP.update_host_link(g_ST, user_vm_link_list, d_ST.host_link_list)
        print()
    ...
    while (count < 12):
        ...
        del_high_threshold_host_list(d_ST, vm_vm_list)

        algorithm_VMDM.update_host_link(g_ST, vm_link_link_list, d_ST.host_link_list)
        algorithm_VMDM.update_host_list(vm_vm_list, d_ST.host_list)

        r = result_ST.Result(result_id=str(uuid.uuid1()),
                               timestamps=clock,
                               power_consumption=power_consumption.get_datacenter_power_consumption(d_ST.host_list),
                               bandwidth_consumption=bandwidth_consumption.get_datacenter_bandwidth_consumption(g_ST,
                                                                                                                d_ST.host_link_list),
                               active_host_num=active_host_number.get_active_host_number(d_ST.host_list),
                               migration_cost=migration_cost)
        print(
            "--------------------------------------------------------------------------------------------------------------")
        print("时间为：" + str(clock), "功耗为：" + str(power_consumption.get_datacenter_power_consumption(d_ST.host_list)),
              "链路资源消耗为：" + str(bandwidth_consumption.get_datacenter_bandwidth_consumption(g_ST,
                                                                                          d_ST.host_link_list)),
              "激活服务器数量为：" + str(active_host_number.get_active_host_number(d_ST.host_list)),
              "迁移代价为：" + str(migration_cost))

        result_ST.add_result_data(r)
        count = count + 1
        clock = clock + 2
...
def get_max_capacity_host(d, vm):
    ...
    host_cpu_free_list = []
    for host in avail_host_list:
        host_cpu_free = host.cpu_sum - host.cpu_used
        host_cpu_free_list.append(host_cpu_free)
    return avail_host_list[host_cpu_free_list.index(max(host_cpu_free_list))]
...
def get_high_threshold_host_list(d):
    host_list = d.host_list
    high_threshold_host_list = []
    for host in host_list:
        if((host.cpu_used * 1.0 / host.cpu_sum) > 0.6 or (host.mem_used * 1.0 / host.mem_sum) > 0.6):
...
def main():
    ...
    while (count < 8):
        ...
        active_host_num_sample = active_host_num + random.randint(-2, 1)
...
def del_high_threshold_host_list(d, vm_list):
    global migration_cost
    high_threshold_host_list = get_high_threshold_host_list(d)
